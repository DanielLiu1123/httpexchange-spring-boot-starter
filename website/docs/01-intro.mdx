...
- Migration to `@HttpExchange` Annotations

    Spring's declarative HTTP client model is based on `@HttpExchange` and composed annotations such as
    `@GetExchange` and `@PostExchange`.
    For users migrating from `Spring Cloud OpenFeign`, this means moving from Spring Web mapping annotations
    to the `@HttpExchange` annotation model.

## Quick Start

<Tabs groupId="build-tool">
    <TabItem value="gradle" label="Gradle">
        <Tabs groupId="boot-version">
            <TabItem value="sb4" label="Spring Boot ≥ 4.0.0">
                ```groovy
            ...
            <TabItem value="sb3" label="Spring Boot < 4.0.0">
                ```groovy
                implementation("io.github.danielliu1123:httpexchange-spring-boot-starter:<latest>")
                ```
            </TabItem>
